classdef ExampleHelperGazeboModel_corrected < handle
    %ExampleHelperGazeboModel_corrected - Class controlling individual models in Gazebo
    %   M = ExampleHelperGazeboModel_corrected(varargin) creates an object that allows for easy
    ...
    %   retrieve it.
    %
    %   ExampleHelperGazeboModel_corrected methods:
    ...
    %
    %   ExampleHelperGazeboModel_corrected properties:
    ...
    methods
        function obj = ExampleHelperGazeboModel_corrected(varargin)
            %ExampleHelperGazeboModel_corrected Constructor
            
            % Determine model to retrieve and how to get it
            obj.Name = varargin{1};
            if nargin == 1
                try
                    obj.ModelObj = xmlread([obj.Name '.sdf']);
                catch
                    obj.ModelObj = com.mathworks.xml.XMLUtils.createDocument('sdf');
        ...
        function out = getModelXml(obj)
            ...
            out = xmlwrite(obj.ModelObj);
        end
        
        function saveLocalModel(obj, filename)
            %saveLocalModel - Writes SDF to a local file
            
            xmlwrite(filename, obj.ModelObj);
        end
            ...
            switch linktype
                ...
                case 'box'
                    dim = type.appendChild(obj.ModelObj.createElement('size'));
                     dim.appendChild(obj.ModelObj.createTextNode(num2str(param)));
